"""
Track Environment Version 10
Full quadrotor tracking environment with a moving target.
Ver10 modifications: Based on Ver8 with enhanced randomization and reward updates:
...
class TrackEnvVer10(env_base.Env[TrackStateVer10]):
    """Quadrotor tracking environment Ver10 - based on Ver8 with enhanced randomization and reward updates."""
    ...
    def reset(
        self, key: chex.PRNGKey, state: Optional[TrackStateVer10] = None, quad_params: Optional[ExtendedQuadrotorParams] = None):
    ...
    def _get_obs(self, state: TrackStateVer10) -> jax.Array:
        """Get observation from state.
        
        Ver10修改：继承Ver8，移除观测延迟，直接使用真实状态值
    ...
    def _step(
        self, state: TrackStateVer10, action: jax.Array, key: chex.PRNGKey
    ) -> EnvTransition:
    ...
    def _compute_reward(
        self, last_state: TrackStateVer10, next_state: TrackStateVer10
    ) -> jax.Array:
        """计算奖励 - 基于 agile_lossVer7 算法，加入推力惩罚
        奖励设计：
