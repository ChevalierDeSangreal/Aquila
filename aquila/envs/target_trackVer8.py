"""
Track Environment Version 8
Full quadrotor tracking environment with a moving target.
Ver8 modifications: Based on Ver6 with enhanced randomization:
...
class TrackEnvVer8(env_base.Env[TrackStateVer8]):
    """Quadrotor tracking environment Ver8 - based on Ver6 with enhanced randomization."""
    ...
    def reset(
        self, key: chex.PRNGKey, state: Optional[TrackStateVer8] = None, quad_params: Optional[ExtendedQuadrotorParams] = None):
    ...
    def _get_obs(self, state: TrackStateVer8) -> jax.Array:
        """Get observation from state.
        
        Ver8修改：继承Ver6，移除观测延迟，直接使用真实状态值
    ...
    def _step(
        self, state: TrackStateVer8, action: jax.Array, key: chex.PRNGKey
    ) -> EnvTransition:
    ...
    def _compute_reward(
        self, last_state: TrackStateVer8, next_state: TrackStateVer8
    ) -> jax.Array:
