"""
Hover Environment Version 4
Full quadrotor hovering environment at the origin.
Ver4 modifications: Based on HoverVer3, with velocity removed from observation.
Ver3 modifications (inherited): Based on HoverVer2, with adjusted loss weight design.
Ver2 modifications (inherited): Based on HoverVer1, with modified reward weight design.
...
Ver3 specific changes:
- Adjusted loss weight design compared to Ver2
Ver4 specific changes:
- Removed velocity from observation (only gravity direction and relative position remain)
...
class HoverEnvVer4(env_base.Env[HoverStateVer4]):
    """Quadrotor hovering environment Ver4 - hover at origin, based on HoverVer3 but with velocity removed from observation."""
    ...
    def reset(
        self, key: chex.PRNGKey, state: Optional[HoverStateVer4] = None, quad_params: Optional[ExtendedQuadrotorParams] = None):
    ...
    def _get_obs(self, state: HoverStateVer4) -> jax.Array:
        """Get observation from state.
        
        Ver4修改：基于HoverVer3，移除了速度观测，只保留重力方向和相对位置
    ...
    def _step(
        self, state: HoverStateVer4, action: jax.Array, key: chex.PRNGKey
    ) -> EnvTransition:
    ...
    def _compute_reward(
        self, last_state: HoverStateVer4, next_state: HoverStateVer4
    ) -> jax.Array:
        """计算奖励 - 悬停任务奖励函数 (Ver4: 基于Ver3，移除了速度观测)
        奖励设计：
        1. 位置损失：到原点的水平距离
        2. 高度损失：与目标高度的偏差
        3. 速度损失：速度模长（应该接近0）
        ...
        Ver3相对于Ver2的修改：在Ver2基础上进一步调整了各项损失的权重设计，以优化训练效果。
        """
        # 获取状态信息
        ...
        # 总损失 - 悬停任务权重调整 (Ver3: 基于Ver2调整了权重设计)
        # - 位置损失：最高权重，确保在原点附近悬停
        # - 高度损失：高权重，保持目标高度
        # - 速度损失：高权重，保持静止
        # - 姿态损失：中等权重，保持水平姿态
        ...
        # Ver4相对于Ver3：移除了速度观测，但奖励函数保持不变
        total_loss = (
            10 * position_loss +      # 位置损失：最高权重
            100 * height_loss +        # 高度损失：最高权重
            5 * velocity_loss +       # 速度损失：高权重
            10 * ori_loss +           # 姿态损失：中等权重
            0.8 * action_loss +          # 动作损失：低权重
            0.1 * omega_loss +           # 角速度损失：低权重
            0.5 * thrust_loss            # 推力超限损失：中等权重
        )
