"""
Hover Environment Version 3
Full quadrotor hovering environment at the origin.
Ver3 modifications: Based on HoverVer2, with adjusted loss weight design.
...
class HoverEnvVer3(env_base.Env[HoverStateVer3]):
    """Quadrotor hovering environment Ver3 - hover at origin, based on HoverVer2 but with adjusted loss weight design."""
    ...
    def reset(
        self, key: chex.PRNGKey, state: Optional[HoverStateVer3] = None, quad_params: Optional[ExtendedQuadrotorParams] = None):
    ...
    def _get_obs(self, state: HoverStateVer3) -> jax.Array:
        """Get observation from state.
        
        Ver3修改：基于HoverVer2，使用Quadrotor（不包含Kp参数）
    ...
    def _step(
        self, state: HoverStateVer3, action: jax.Array, key: chex.PRNGKey
    ) -> EnvTransition:
    ...
    def _compute_reward(
        self, last_state: HoverStateVer3, next_state: HoverStateVer3
    ) -> jax.Array:
        """计算奖励 - 悬停任务奖励函数 (Ver3: 基于Ver2调整了loss权重设计)
        奖励设计：
        1. 位置损失：到原点的水平距离
        2. 高度损失：与目标高度的偏差
        3. 速度损失：速度模长（应该接近0）
        ...
        # Ver3相对于Ver2：在Ver2基础上进一步调整了各项损失的权重比例，以优化训练效果
        total_loss = (
