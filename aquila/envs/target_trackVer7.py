"""
Track Environment Version 7
Full quadrotor tracking environment with a target moving on x-axis.
Ver7 modification: Based on Ver6, uses quadrotor_objVer2 with Kp randomization support.
...
from aquila.objects.quadrotor_objVer2 import Quadrotor, QuadrotorState, QuadrotorParams
...
class ExtendedQuadrotorParams(QuadrotorParams):
    """扩展的QuadrotorParams Ver7，包含mass、gravity和external_force以便支持动态扰动
    Ver7: 兼容quadrotor_objVer2，支持Kp随机化"""
    mass: float = 1.0  # [kg]
...
class TrackEnvVer7(env_base.Env[TrackStateVer7]):
    """Quadrotor tracking environment Ver7 - based on Ver6, uses quadrotor_objVer2 with Kp randomization support."""
    ...
    def reset(
        self, key: chex.PRNGKey, state: Optional[TrackStateVer7] = None, quad_params: Optional[ExtendedQuadrotorParams] = None):
        ...
        if quad_params is None:
            ...
            # 转换为扩展的参数类Ver7，添加mass、gravity和Kp
            quad_params = ExtendedQuadrotorParams(
                ...
                Kp=randomized_params.Kp,  # Ver7: 添加Kp随机化支持
    ...
    def _get_obs(self, state: TrackStateVer7) -> jax.Array:
        """Get observation from state.
        
        Ver7修改：使用quadrotor_objVer2，支持Kp随机化
    ...
    def _step(
        self, state: TrackStateVer7, action: jax.Array, key: chex.PRNGKey
    ) -> EnvTransition:
        # 保存原始action (tanh输出为[-1,1]范围)
        action_raw = action
        
        # Ver7: 将tanh输出的action [-1, 1] 映射到实际范围
        ...
        # ⚠️  Ver7: 使用当前状态的实际thrust_max（参数随机化后的值，包括Kp）
        ...
        # ⚠️  Ver7: 使用当前状态的实际omega_max（参数随机化后的值）
        ...
        # Ver7: 直接传递ExtendedQuadrotorParams（包含mass、gravity、external_force、Kp）
        # quadrotor_objVer2的step方法支持ExtendedQuadrotorParams和Kp随机化
        quadrotor_state = self.quadrotor.step(
            state.quadrotor_state, f_1, omega_1, dt_1, 
            drag_params=None,  # 使用默认drag_params
            quad_params=state.quad_params  # 使用ExtendedQuadrotorParams (Ver7)
        )
    ...
    def _compute_reward(
        self, last_state: TrackStateVer7, next_state: TrackStateVer7
    ) -> jax.Array:
