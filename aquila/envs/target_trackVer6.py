"""
Track Environment Version 6
Full quadrotor tracking environment with a target moving on x-axis.
Ver6 modification: Based on Ver5 but removed observation delay - now uses true state values directly.
...
class TrackEnvVer6(env_base.Env[TrackStateVer6]):
    """Quadrotor tracking environment Ver6 - based on Ver5 but removed observation delay, now uses true state values directly."""
    ...
    def reset(
        self, key: chex.PRNGKey, state: Optional[TrackStateVer6] = None, quad_params: Optional[ExtendedQuadrotorParams] = None):
    ...
    def _get_obs(self, state: TrackStateVer6) -> jax.Array:
        """Get observation from state.
        
        Ver6修改：移除观测延迟，直接使用真实状态值
    ...
    def _step(
        self, state: TrackStateVer6, action: jax.Array, key: chex.PRNGKey
    ) -> EnvTransition:
    ...
    def _compute_reward(
        self, last_state: TrackStateVer6, next_state: TrackStateVer6
    ) -> jax.Array:
        ...
        total_loss = (
            ...
            0.3 * action_loss +         # 动作损失：指数增长，权重降低
            0.3 * omega_loss             # 角速度损失：指数增长，权重降低
        )
