...
def train(
    ...
    validation_fn: Optional[Callable] = None,  # Ver3新增：验证函数
    validation_interval: int = 10,  # Ver3新增：验证间隔（每N个epoch验证一次）
):
    """
    Ver3: 基于Ver2，添加验证功能
    - 支持自定义验证函数，每隔 validation_interval 个epoch调用一次
    - 验证函数接收 (train_state, epoch_idx) 并返回验证指标字典
    - 继续支持双输出网络（Ver14/Ver16支持）
    """
    ...
    def _train(runner_state: RunnerState):
        def epoch_fn(epoch_state: RunnerState, _unused):
            ...
            # env_state是LogEnvState，包含env_state字段，由于VecEnv，env_state是向量化的TrackStateVer16
        ...
        # Ver3: 使用Python for循环代替jax.lax.scan，支持验证
        final_runner_state = runner_state
        losses = []
        
        print(f"[bpttVer3.train] 开始训练循环（使用Python for循环）...")
        import sys
        sys.stdout.flush()
        
        for epoch_idx in range(num_epochs):
            # 训练一个epoch
            final_runner_state, loss = epoch_fn(final_runner_state, None)
            losses.append(loss)
            
            # Ver3: 验证逻辑
            if validation_fn is not None and (epoch_idx + 1) % validation_interval == 0:
                print(f"\n[bpttVer3] Epoch {epoch_idx + 1}/{num_epochs} - 开始验证...")
                sys.stdout.flush()
                
                # 调用验证函数
                val_start_time = time.time()
                val_metrics = validation_fn(final_runner_state.train_state, epoch_idx + 1)
                val_time = time.time() - val_start_time
                
                print(f"[bpttVer3] 验证完成，耗时 {val_time:.2f}s")
        ...
        return {"runner_state": final_runner_state, "metrics": losses}

    # intialize environments
    print(f"[bpttVer3.train] 正在初始化 {num_envs} 个并行环境...")
    print(f"[bpttVer3.train] 动作重复设置: 每 {action_repeat} 步获取一次新动作")
    print(f"[bpttVer3.train] 动作-状态缓冲区大小: {buffer_size}")
    print(f"[bpttVer3.train] 支持双输出网络（Ver16）：动作 + 辅助输出")
    print(f"[bpttVer3.train] 验证间隔: 每 {validation_interval} 个epoch")
    import sys
    sys.stdout.flush()
    ...
    print(f"[bpttVer3.train] 环境初始化完成，观测形状: {obs.shape}")
    print(f"[bpttVer3.train] 动作-状态缓冲区形状: {action_obs_buffer_init.shape}")
    print(f"[bpttVer3.train] 展平后输入形状: {action_obs_buffer_flat.shape}")
    print(f"[bpttVer3.train] 初始动作形状: {initial_action.shape}")
    print(f"[bpttVer3.train] 开始训练（不使用JIT，以支持Python验证逻辑）...")
    sys.stdout.flush()

    result = _train(runner_state)
    
    print(f"[bpttVer3.train] 训练已结束")
    sys.stdout.flush()
