...
def lead_lag_discrete(u_curr: jnp.ndarray,
                      u_prev: jnp.ndarray,
                      dt: jnp.ndarray,
                      k_lead: float,
                      tau_lag: float,
                      y_prev: jnp.ndarray) -> jnp.ndarray:
    """
    Minimal discrete lead-lag: y = LPF( u + k*(u-u_prev)/dt ), LPF(alpha=dt/(tau+dt)).
    - u_curr, u_prev: current/previous input
    - dt: step time
    - k_lead: lead coefficient k
    - tau_lag: lag time constant tau (0 disables filtering)
    - y_prev: previous filter output (same shape as u)
    Returns new output y.
    """
    du = (u_curr - u_prev) / jnp.maximum(dt, 1e-6)
    u_lead = u_curr + k_lead * du
    alpha = jnp.where(tau_lag > 0.0, dt / (tau_lag + dt), 1.0)
    y = (1.0 - alpha) * y_prev + alpha * u_lead
    return y