...
# aquila/scripts/train_trackVer10.py -> ../../ -> Aquila project root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))

from aquila.envs.target_trackVer10 import TrackEnvVer10  # 使用TrackEnvVer10（增强随机化和奖励更新版本）
...
def main():
    ...
    # ==================== Environment Setup ====================
    # Create env - 使用TrackEnvVer10环境（增强随机化和奖励更新版本）
    env = TrackEnvVer10(
    ...
    if choice == 1:
        ...
        # 使用加载的参数
        policy_file = 'aquila/param/trackVer10_policy.pkl'  # 使用Ver10的模型文件
    ...
    # ==================== TensorBoard Setup ====================
    # 创建tensorboard日志目录
    log_dir = f'runs/trackVer10_{time.strftime("%Y%m%d_%H%M%S")}'  # 使用Ver10的日志目录
    ...
    print(f"\n{'='*60}")
    print(f"开始训练追踪任务 (TrackVer10 - 增强随机化和奖励更新版本)...")
    ...
    # ==================== Print Summary ====================
    print(f"\n{'='*60}")
    print(f"追踪任务训练完成！(TrackVer10 - 增强随机化和奖励更新版本)")
    print(f"{'='*60}")
    ...
    checkpoint_data = {
        ...
        'env_config': {
            ...
            # Ver10 增强了随机化：目标位置y和z随机，roll/pitch±30°随机，yaw完全随机，初始速度0-0.5m/s随机方向，目标速度每episode随机，奖励中加入推力惩罚
    ...
    # 保存为pickle文件
    checkpoint_path = 'aquila/param/trackVer10_policy.pkl'  # 使用Ver10的文件名
    ...
    backup_path = f'aquila/param/trackVer10_policy_{timestamp}.pkl'  # 使用Ver10的文件名
