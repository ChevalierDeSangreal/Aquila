...
# coding: utf-8
"""
Training script for TrackEnvVer7
Ver7: Based on Ver6, uses quadrotor_objVer2 with Kp randomization support
"""
...
# aquila/scripts/train_trackVer7.py -> ../../ -> Aquila project root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))

from aquila.envs.target_trackVer7 import TrackEnvVer7  # 使用TrackEnvVer7（基于Ver6，使用quadrotor_objVer2，支持Kp随机化）
...
def main():
    ...
    # ==================== Environment Setup ====================
    # Create env - 使用TrackEnvVer7环境（基于Ver6，使用quadrotor_objVer2，支持Kp随机化）
    env = TrackEnvVer7(
        max_steps_in_episode=1000,  # 追踪任务的最大步数
        dt=0.01,  # 使用完整四旋翼的默认时间步长
        delay=0.03,  # 可选执行延迟（Ver7默认仍支持）
        omega_std=0.1,
        ...
        max_speed=20.0,  # m/s
        # Parameter randomization (quadrotor) Ver7: 包括Kp随机化
    ...
    if choice == 1:
        ...
        # 使用加载的参数
        policy_file = 'aquila/param/trackVer7_policy.pkl'  # 使用Ver7的模型文件
    ...
    # ==================== TensorBoard Setup ====================
    # 创建tensorboard日志目录
    log_dir = f'runs/trackVer7_{time.strftime("%Y%m%d_%H%M%S")}'  # 使用Ver7的日志目录
    ...
    print(f"\n{'='*60}")
    print(f"开始训练追踪任务 (TrackVer7 - 基于Ver6，使用quadrotor_objVer2，支持Kp随机化)...")
    ...
    # ==================== Print Summary ====================
    print(f"\n{'='*60}")
    print(f"追踪任务训练完成！(TrackVer7 - 基于Ver6，使用quadrotor_objVer2，支持Kp随机化)")
    print(f"{'='*60}")
    ...
    checkpoint_data = {
        ...
        'env_config': {
            ...
            # Ver7 移除了观测延迟，所以不再有 obs_tau_pos, obs_tau_vel, obs_tau_R
    ...
    # 保存为pickle文件
    checkpoint_path = 'aquila/param/trackVer7_policy.pkl'  # 使用Ver7的文件名
    ...
    backup_path = f'aquila/param/trackVer7_policy_{timestamp}.pkl'  # 使用Ver7的文件名
