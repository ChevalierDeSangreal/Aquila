...
def test_policy():
    ...
    policy_file = 'aquila/param/trackVer13_policy.pkl'
    ...
    state = TrackStateVer13(
        ...
        target_acceleration=0.0,  # Ver13新增：目标加速度（圆形轨迹不需要加速度）
    ...
    for step in range(max_steps):
        ...
        # 计算目标加速度（圆形运动的向心加速度）
        target_acc = -(circle_speed**2 / circle_radius) * jnp.array([
            jnp.cos(circle_angle),
            jnp.sin(circle_angle),
            0.0
        ])
    ...
    output_file = 'aquila/output/test_trackVer13_circle_data.pkl'
    ...
    plt.tight_layout()
    trajectory_file = 'aquila/output/trackVer13_circle_trajectory_3d.png'
    ...
    plt.tight_layout()
    analysis_file = 'aquila/output/trackVer13_circle_data_analysis.png'
    ...
    axes[0, 1].grid(True, alpha=0.3)
    axes[0, 1].axhline(y=circle_speed, color='r', linestyle='--', alpha=0.5, label=f'Expected speed ({circle_speed} m/s)')
    axes[0, 1].legend()
    ...
    expected_acceleration = circle_speed**2 / circle_radius  # 向心加速度
    ...
    axes[1, 1].grid(True, alpha=0.3)
    axes[1, 1].axhline(y=expected_acceleration, color='r', linestyle='--', alpha=0.5, 
                       label=f'Expected centripetal acc ({expected_acceleration:.3f} m/s²)')
    axes[1, 1].legend()
    
    plt.tight_layout()
    target_analysis_file = 'aquila/output/trackVer13_circle_target_analysis.png'
