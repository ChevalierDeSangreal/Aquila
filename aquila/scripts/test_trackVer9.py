...
from aquila.envs.target_trackVer9 import TrackEnvVer9
...
def run_episode(env, policy, params, buffer_size, action_repeat, max_steps=2000, key=None):
    ...
    # 访问被wrapper包装的环境：env -> NormalizeActionWrapper -> MinMaxObservationWrapper -> TrackEnvVer9
...
def visualize_data(data, output_dir):
    ...
    plt.suptitle('TrackVer9 Test Results - Data Analysis', 
...
def main():
    ...
    if not os.path.exists(policy_file):
        ...
        print(f"   请先运行 train_trackVer9.py 进行训练")
        return
    ...
    env = TrackEnvVer9(
        ...
        target_speed_max=0,  # m/s (目标最大速度上限，实际每episode会在0-1之间随机)
