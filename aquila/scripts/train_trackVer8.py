...
# aquila/scripts/train_trackVer8.py -> ../../ -> Aquila project root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))

from aquila.envs.target_trackVer8 import TrackEnvVer8  # 使用TrackEnvVer8（增强随机化版本）
...
def main():
    ...
    # ==================== Environment Setup ====================
    # Create env - 使用TrackEnvVer8环境（增强随机化版本）
    env = TrackEnvVer8(
    ...
    if choice == 1:
        ...
        # 使用加载的参数
        policy_file = 'aquila/param/trackVer8_policy.pkl'  # 使用Ver8的模型文件
    ...
    # ==================== TensorBoard Setup ====================
    # 创建tensorboard日志目录
    log_dir = f'runs/trackVer8_{time.strftime("%Y%m%d_%H%M%S")}'  # 使用Ver8的日志目录
    ...
    print(f"\n{'='*60}")
    print(f"开始训练追踪任务 (TrackVer8 - 增强随机化版本)...")
    ...
    # ==================== Print Summary ====================
    print(f"\n{'='*60}")
    print(f"追踪任务训练完成！(TrackVer8 - 增强随机化版本)")
    print(f"{'='*60}")
    ...
    checkpoint_data = {
        ...
        'env_config': {
            ...
            # Ver8 增强了随机化：目标位置y和z随机，roll/pitch随机，目标速度每episode随机
    ...
    # 保存为pickle文件
    checkpoint_path = 'aquila/param/trackVer8_policy.pkl'  # 使用Ver8的文件名
    ...
    backup_path = f'aquila/param/trackVer8_policy_{timestamp}.pkl'  # 使用Ver8的文件名
