...
# aquila/scripts/train_trackVer5.py -> ../../ -> Aquila project root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))

from aquila.envs.target_trackVer5 import TrackEnvVer5  # 使用TrackEnvVer5（动作-状态缓冲区 + 完整四旋翼模型）
...
def main():
    ...
    # ==================== Environment Setup ====================
    # Create env - 使用TrackEnvVer5环境（动作-状态缓冲区 + 完整四旋翼模型）
    env = TrackEnvVer5(
    ...
    if choice == 1:
        ...
        # 使用加载的参数
        policy_file = 'aquila/param/trackVer5_policy.pkl'  # 使用Ver5的模型文件
    ...
    # ==================== TensorBoard Setup ====================
    # 创建tensorboard日志目录
    log_dir = f'runs/trackVer5_{time.strftime("%Y%m%d_%H%M%S")}'  # 使用Ver5的日志目录
    ...
    print(f"\n{'='*60}")
    print(f"开始训练追踪任务 (TrackVer5 - 动作-状态缓冲区模式 + 完整四旋翼模型)...")
    ...
    # ==================== Print Summary ====================
    print(f"\n{'='*60}")
    print(f"追踪任务训练完成！(TrackVer5 - 动作-状态缓冲区模式 + 完整四旋翼模型)")
    print(f"{'='*60}")
    ...
    checkpoint_data = {
        ...
        'env_config': {
            ...
            'obs_tau_pos': env.obs_tau_pos,
            'obs_tau_vel': env.obs_tau_vel,
            'obs_tau_R': env.obs_tau_R,
    ...
    # 保存为pickle文件
    checkpoint_path = 'aquila/param/trackVer5_policy.pkl'  # 使用Ver5的文件名
    ...
    backup_path = f'aquila/param/trackVer5_policy_{timestamp}.pkl'  # 使用Ver5的文件名
