...
def run_episode(env, policy, params, buffer_size, action_repeat, max_steps=2000, key=None):
    ...
    # 访问被wrapper包装的环境：env -> NormalizeActionWrapper -> MinMaxObservationWrapper -> TrackEnvVer6
...
def visualize_data(data, output_dir):
    ...
    plt.suptitle('TrackVer6 Test Results - Data Analysis', 
...
def main():
    ...
    # ==================== Load Policy ====================
    policy_file = 'aquila/param/trackVer6_policy.pkl'
    
    if not os.path.exists(policy_file):
        print(f"❌ 错误: 找不到训练好的模型文件: {policy_file}")
        print(f"   请先运行 train_trackVer6.py 进行训练")
        return
