...
def ensure_trajectory_reasonable(trajectory: AcceleratedLinearTrajectory) -> AcceleratedLinearTrajectory:
    """
    确保直线轨迹参数合理（Ver10没有边界约束，只需确保参数在合理范围内）
    
    Args:
        trajectory: 原始直线轨迹生成器
    ...
    # 确保最大速度在合理范围内
    max_speed = np.clip(trajectory.max_speed, 0.1, 5.0)
    
    # 确保加速度在合理范围内
    acceleration = np.clip(trajectory.acceleration, 0.1, 2.0)
    
    # 确保z坐标在合理范围内（NED坐标系，z通常为负值）
    start_pos = np.array(trajectory.start_position)
    start_pos[2] = np.clip(start_pos[2], -10.0, -0.5)
    
    # 获取方向向量（转换为numpy数组）
    direction = np.array(trajectory.direction)
    
    # 创建新的直线轨迹
    return AcceleratedLinearTrajectory(
        start_position=tuple(start_pos),
        direction=tuple(direction),
        max_speed=float(max_speed),
        acceleration=float(acceleration),
        constant_velocity_duration=trajectory.constant_velocity_duration
    )


def run_test_episode(env, policy_apply, params, key, action_repeat, buffer_size, 
                     trajectory: AcceleratedLinearTrajectory = None, verbose=False):
    """
    ...
    Args:
        ...
        trajectory: 直线轨迹生成器，如果提供则覆盖环境的默认目标运动
        verbose: 是否打印详细信息
    """
...
def main():
    ...
    # 创建直线轨迹：匀加速到最大速度后匀速运动
    trajectory = create_trajectory(
        'accelerated_linear',
        start_position=(0.0, 0.0, -2.0),  # 起始位置（NED坐标系）
        direction=(1.0, 0.0, 0.0),        # 运动方向（X方向，即North方向）
        max_speed=1.0,                     # 最大速度（m/s）
        acceleration=0.5,                  # 加速度（m/s²）
        constant_velocity_duration=20.0    # 匀速运动持续时间（秒）
    )
    
    print(f"\n{'='*60}")
    print(f"Linear Trajectory Configuration:")
    print(f"{'='*60}")
    ...
    if tracking_passed:
        print(f"\n🎉 Tracking test PASSED! The policy successfully tracks the linear trajectory target.")
    else:
