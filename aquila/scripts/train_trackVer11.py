...
# aquila/scripts/train_trackVer11.py -> ../../ -> Aquila project root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))

from aquila.envs.target_trackVer11 import TrackEnvVer11  # 使用TrackEnvVer11（带边界约束的追踪环境）
...
def main():
    ...
    # ==================== Environment Setup ====================
    # Create env - 使用TrackEnvVer11环境（带边界约束的追踪环境）
    env = TrackEnvVer11(
        ...
        target_height=2.0,  # m (高度2米，实际会在边界内随机初始化)
        target_init_distance_min=0.5,  # m (x轴上的初始距离最小值)
        target_init_distance_max=1.5,  # m (x轴上的初始距离最大值)
        target_speed_max=2.0,  # m/s (目标最大速度上限，实际每episode会在0-2之间随机)
        reset_distance=100.0,  # m (重置距离阈值)
        max_speed=20.0,  # m/s
        # Boundary parameters (NED坐标系：z范围[0, boundary_z]，完全在地面以下)
        boundary_x=10.0,  # m (x方向全长，中心位于原点)
        boundary_y=10.0,  # m (y方向全长，中心位于原点)
        boundary_z=10.0,  # m (z方向全长，从地面z=0到z=boundary_z)
        boundary_penalty_distance=1.0,  # m (边界惩罚距离阈值)
        boundary_penalty_max=100.0,  # 超出边界的最大惩罚值
    ...
    if choice == 1:
        ...
        # 使用加载的参数
        policy_file = 'aquila/param/trackVer11_policy.pkl'  # 使用Ver11的模型文件
    ...
    # ==================== TensorBoard Setup ====================
    # 创建tensorboard日志目录
    log_dir = f'runs/trackVer11_{time.strftime("%Y%m%d_%H%M%S")}'  # 使用Ver11的日志目录
    ...
    print(f"\n{'='*60}")
    print(f"开始训练追踪任务 (TrackVer11 - 带边界约束的追踪环境)...")
    ...
    print(f"Boundary: x=[{-env.boundary_half_x:.1f}, {env.boundary_half_x:.1f}], "
          ...
          f"z=[0.0, {env.boundary_z:.1f}] (NED: z-positive is downward)")
    ...
    # ==================== Print Summary ====================
    print(f"\n{'='*60}")
    print(f"追踪任务训练完成！(TrackVer11 - 带边界约束的追踪环境)")
    print(f"{'='*60}")
    ...
    checkpoint_data = {
        ...
        'env_config': {
            ...
            'boundary_x': env.boundary_half_x * 2.0,
            'boundary_y': env.boundary_half_y * 2.0,
            'boundary_z': env.boundary_z,
            'boundary_penalty_distance': env.boundary_penalty_distance,
            'boundary_penalty_max': env.boundary_penalty_max,
        }
    }
    ...
    # 保存为pickle文件
    checkpoint_path = 'aquila/param/trackVer11_policy.pkl'  # 使用Ver11的文件名
    ...
    backup_path = f'aquila/param/trackVer11_policy_{timestamp}.pkl'  # 使用Ver11的文件名
