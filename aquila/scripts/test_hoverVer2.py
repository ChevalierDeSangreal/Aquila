...
def create_initial_state_at_target(env, key, use_easy_init=False):
    """创建初始状态
    
    Args:
        env: 环境
        key: 随机数key
        use_easy_init: 如果为True，使用简单初始条件（接近目标点）
                      如果为False，使用和训练时相同的随机化初始条件
    """
    # 获取默认的quad_params（带参数随机化）
    keys = jax.random.split(key, 7)
    key_reset, key_randomize, key_pos, key_angles, key_vel_dir, key_vel_mag, key_omega = keys
    ...
    # 转换为扩展的参数类（hoverVer2使用Quadrotor，不包含Kp参数）
    ...
    if use_easy_init:
        ...
        # ========== 和训练时相同的随机化初始条件 ==========
        # 位置：距离原点0~0.5m范围内随机（与训练时一致）
        pos_keys = jax.random.split(key_pos, 3)
        theta = jax.random.uniform(pos_keys[0], shape=(), minval=0.0, maxval=2.0 * jnp.pi)
        phi = jnp.arccos(jax.random.uniform(pos_keys[1], shape=(), minval=-1.0, maxval=1.0))
        r = jax.random.uniform(pos_keys[2], shape=(), minval=0.0, maxval=env.init_pos_range)
        
        # 球坐标转笛卡尔坐标
        x_offset = r * jnp.sin(phi) * jnp.cos(theta)
        y_offset = r * jnp.sin(phi) * jnp.sin(theta)
        z_offset = r * jnp.cos(phi)
        offset = jnp.array([x_offset, y_offset, z_offset])
        p = env.hover_origin + offset
        
        # 速度：随机方向，大小在0~2m/s（与训练时一致）
        vel_dir_keys = jax.random.split(key_vel_dir, 2)
        vel_theta = jax.random.uniform(vel_dir_keys[0], shape=(), minval=0.0, maxval=2.0 * jnp.pi)
        vel_phi = jnp.arccos(jax.random.uniform(vel_dir_keys[1], shape=(), minval=-1.0, maxval=1.0))
        vel_mag = jax.random.uniform(key_vel_mag, shape=(), minval=0.0, maxval=2.0)
        
        vel_x = vel_mag * jnp.sin(vel_phi) * jnp.cos(vel_theta)
        vel_y = vel_mag * jnp.sin(vel_phi) * jnp.sin(vel_theta)
        vel_z = vel_mag * jnp.cos(vel_phi)
        v = jnp.array([vel_x, vel_y, vel_z])
        
        # 姿态：roll和pitch在±30°，yaw在±180°（与训练时一致）
        angle_keys = jax.random.split(key_angles, 3)
        roll = jax.random.uniform(angle_keys[0], shape=(), minval=-jnp.pi/6, maxval=jnp.pi/6)
        pitch = jax.random.uniform(angle_keys[1], shape=(), minval=-jnp.pi/6, maxval=jnp.pi/6)
        yaw = jax.random.uniform(angle_keys[2], shape=(), minval=-jnp.pi, maxval=jnp.pi)
    ...
    # 创建state
    from aquila.envs.hoverVer2 import HoverStateVer2
...
def run_episode(env, policy, params, state, max_steps=1000, action_repeat=2, buffer_size=50):
    """运行一个episode，记录所有数据
    
    ⚠️ 修复：确保缓冲区更新逻辑与训练时完全一致
    关键点：在同一个step内，使用同一个obs更新缓冲区两次（空动作+obs，新动作+obs），
           然后才执行env.step获取新obs
    """
    # 数据记录
    positions = []
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Testing HoverVer2 Policy (50Hz action update frequency)")
    print(f"{'='*60}")
    ...
    # 保存数据
    data_path = os.path.join(output_dir, 'test_hoverVer2_data.pkl')
    ...
    # 3D轨迹图
    trajectory_path = os.path.join(output_dir, 'hoverVer2_trajectory_3d.png')
    plot_trajectory_3d(data, trajectory_path)
    
    # 数据分析图
    analysis_path = os.path.join(output_dir, 'hoverVer2_data_analysis.png')
