...
# aquila/scripts/train_trackVer6.py -> ../../ -> Aquila project root
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '../..'))

from aquila.envs.target_trackVer6 import TrackEnvVer6  # 使用TrackEnvVer6（动作-状态缓冲区 + 完整四旋翼模型，无观测延迟）
...
def main():
    ...
    # ==================== Environment Setup ====================
    # Create env - 使用TrackEnvVer6环境（动作-状态缓冲区 + 完整四旋翼模型，无观测延迟）
    env = TrackEnvVer6(
        max_steps_in_episode=1000,  # 追踪任务的最大步数
        dt=0.01,  # 使用完整四旋翼的默认时间步长
        delay=0.03,  # 可选执行延迟（Ver6默认仍支持）
        omega_std=0.1,
    ...
    if choice == 1:
        ...
        # 使用加载的参数
        policy_file = 'aquila/param/trackVer6_policy.pkl'  # 使用Ver6的模型文件
    ...
    # ==================== TensorBoard Setup ====================
    # 创建tensorboard日志目录
    log_dir = f'runs/trackVer6_{time.strftime("%Y%m%d_%H%M%S")}'  # 使用Ver6的日志目录
    ...
    print(f"\n{'='*60}")
    print(f"开始训练追踪任务 (TrackVer6 - 动作-状态缓冲区模式 + 完整四旋翼模型，无观测延迟)...")
    ...
    # ==================== Print Summary ====================
    print(f"\n{'='*60}")
    print(f"追踪任务训练完成！(TrackVer6 - 动作-状态缓冲区模式 + 完整四旋翼模型，无观测延迟)")
    print(f"{'='*60}")
    ...
    checkpoint_data = {
        ...
        'env_config': {
            ...
            # Ver6 移除了观测延迟，所以不再有 obs_tau_pos, obs_tau_vel, obs_tau_R
    ...
    # 保存为pickle文件
    checkpoint_path = 'aquila/param/trackVer6_policy.pkl'  # 使用Ver6的文件名
    ...
    backup_path = f'aquila/param/trackVer6_policy_{timestamp}.pkl'  # 使用Ver6的文件名
