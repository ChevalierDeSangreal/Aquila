...
def run_test_episode(env, policy_apply, params, key, action_repeat, buffer_size, verbose=False):
...
def main():
    ...
    for episode_idx in range(num_test_episodes):
        ...
        episode_data = run_test_episode(
            ...
            action_repeat, buffer_size, verbose=True
        )
    ...
    print(f"\n{'='*60}")
    print(f"Aggregate Test Results ({num_test_episodes} episodes):")
    print(f"{'='*60}\n")
    ...
    print(f"\n📈 Episode Statistics:")
    print(f"   • Episodes terminated early: {num_terminated} (tracking lost)")
    print(f"   • Episodes completed fully: {num_truncated} (reached max steps)")
    print(f"   • Mean episode length: {total_steps / num_test_episodes:.1f} steps")
    ...
    for i in range(num_test_episodes):
        print(f"Visualizing episode {i + 1}/{num_test_episodes}...")
    ...
    # 3. Tracking success rate per episode
    ax = axes[1, 0]
    success_rates = [np.mean(np.abs(np.array(ep['distances']) - 1.0) < 0.3) * 100 for ep in all_episode_data]
    ax.bar(range(1, num_test_episodes + 1), success_rates, alpha=0.7, color='blue', edgecolor='black')
    ax.axhline(y=70, color='r', linestyle='--', linewidth=2, label='Target (70%)')
    ax.set_xlabel('Episode')
    ax.set_ylabel('Success Rate [%]')
    ax.set_title('Tracking Success Rate (±30cm) per Episode')
    ax.legend()
    ax.grid(True, alpha=0.3)
    
    # 4. Boundary violation rate per episode
    ax = axes[1, 1]
    violation_rates = [ep['boundary_violations'] / ep['num_steps'] * 100 for ep in all_episode_data]
    ax.bar(range(1, num_test_episodes + 1), violation_rates, alpha=0.7, color='green', edgecolor='black')
    ax.axhline(y=1.0, color='r', linestyle='--', linewidth=2, label='Target (<1%)')
    ax.set_xlabel('Episode')
    ax.set_ylabel('Violation Rate [%]')
    ax.set_title('Boundary Violation Rate per Episode')
    ax.legend()
    ax.grid(True, alpha=0.3)
